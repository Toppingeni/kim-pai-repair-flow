...
export function PMExecutionForm({ pmSchedule, onClose }: PMExecutionFormProps) {
  ...
  const handleComplete = () => {
    ...
    if (completedItems < totalItems) {
      toast({
        ...
        description: `ยังเหลือรายการที่ยังไม่ได้ตรวจสอบ ${totalItems - completedItems} รายการ`,
        variant: "destructive",
      });
      return;
    }
  ...
  const handleCreateRepairRequest = () => {
    toast({
      ...
      description: "ระบบได้สร้างใบแจ้งซ่อมจากปัญหาที่พบใน PM แล้ว",
    });
    onClose();
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
            <div>
              <Label className="text-muted-foreground">รหัส PM</Label>
              <p className="font-medium">{pmSchedule.id}</p>
            </div>
            <div>
              <Label className="text-muted-foreground">เครื่องจักร</Label>
              <p className="font-medium">{pmSchedule.machine}</p>
            </div>
            <div>
              <Label className="text-muted-foreground">ประเภท PM</Label>
              <p className="font-medium">{pmSchedule.pmType}</p>
            </div>
            <div>
              <Label className="text-muted-foreground">ผู้รับผิดชอบ</Label>
              <p className="font-medium">{pmSchedule.assignedTo}</p>
            </div>
          </div>
        </CardContent>
      </Card>
        ...
        <CardHeader>
          <CardTitle className="flex items-center justify-between">
            ...
            <Badge variant="outline">
              {completedItems}/{checklist.length} เสร็จ ({Math.round(progressPercentage)}%)
            </Badge>
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          <div className="space-y-3">
            {checklist.map((item, index) => (
              <div key={index} className="flex items-center space-x-3 p-3 border border-border rounded-lg hover:bg-muted/50">
                ...
                <Label 
                  htmlFor={`check-${index}`} 
                  className={`flex-1 cursor-pointer ${checklistStatus[item] ? 'line-through text-muted-foreground' : ''}`}
                >
                  {item}
                </Label>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-lg">รายละเอียดการดำเนินงาน</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="space-y-2">
          ...
          <div className="space-y-3">
            ...
            <RadioGroup value={result} onValueChange={setResult}>
              <div className="flex items-center space-x-2">
                ...
                <Label htmlFor="normal" className="text-status-completed cursor-pointer">
                  ปกติ - ไม่พบปัญหา
                </Label>
              </div>
              <div className="flex items-center space-x-2">
                <RadioGroupItem value="minor-issues" id="minor-issues" />
                <Label htmlFor="minor-issues" className="text-status-pending cursor-pointer">
                  พบปัญหาเล็กน้อย - แก้ไขได้แล้ว
                </Label>
              </div>
              <div className="flex items-center space-x-2">
                <RadioGroupItem value="major-issues" id="major-issues" />
                <Label htmlFor="major-issues" className="text-destructive cursor-pointer">
          ...
          {(result === "minor-issues" || result === "major-issues") && (
            <div className="space-y-2">
              <Label htmlFor="issueDetails">รายละเอียดปัญหา</Label>
              <Textarea 
                id="issueDetails"
          ...
          {result === "major-issues" && (
            <div className="p-4 border border-destructive/20 bg-destructive/5 rounded-lg">
              <div className="flex items-center space-x-2 text-destructive mb-2">
                ...
                <span className="font-medium">ต้องการซ่อมแซม</span>
              </div>
              <p className="text-sm text-muted-foreground mb-3">
                เนื่องจากพบปัญหาร้ายแรง ระบบจะสร้างใบแจ้งซ่อมอัตโนมัติ
              </p>
              <div className="flex items-center space-x-2">
                <Checkbox
                  id="needsRepair"
                  checked={needsRepair}
                  onCheckedChange={(checked) => setNeedsRepair(checked as boolean)}
                />
                <Label htmlFor="needsRepair" className="text-sm">
                  สร้างใบแจ้งซ่อมอัตโนมัติ
                </Label>
              </div>
            </div>
          )}
        ...
        <div className="flex space-x-2">
          {result === "major-issues" && needsRepair && (
            <AlertDialog>
              ...
              <AlertDialogContent>
                <AlertDialogHeader>
                  <AlertDialogTitle>สร้างใบแจ้งซ่อม</AlertDialogTitle>
                  <AlertDialogDescription>
                    ระบบจะสร้างใบแจ้งซ่อมจากปัญหาที่พบใน PM นี้ ต้องการดำเนินการหรือไม่?
                  </AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter>
                  <AlertDialogCancel>ยกเลิก</AlertDialogCancel>
                  <AlertDialogAction onClick={handleCreateRepairRequest}>
                    สร้างใบแจ้งซ่อม
                  </AlertDialogAction>
                </AlertDialogFooter>
              </AlertDialogContent>

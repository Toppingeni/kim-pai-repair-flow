...
export function useRepairActions() {
    ...
    const completeRepair = async (
        ...
        try {
            ...
            const newStatus =
                repairData.result === "success"
                    ? "waiting"
                    : repairData.result === "failed"
                    ? "new"
                    : repairData.result === "pending"
                    ? "pending"
                    : "progress";
    ...
    const retryRepair = async (repairId: string) => {
        ...
        try {
            ...
            toast({
                title: "สร้างใบร้องของงานซ่อมใหม่สำเร็จ",
                description: `สร้างใบร้องของงานซ่อมใหม่สำหรับ ${repairId} เรียบร้อยแล้ว`,
            });
            ...
            toast({
                title: "เกิดข้อผิดพลาด",
                description:
                    "ไม่สามารถสร้างใบร้องของงานซ่อมใหม่ได้ โปรดลองใหม่อีกครั้ง",
                variant: "destructive",
            });
            return { success: false };
        } finally {
